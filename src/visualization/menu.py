...
from src.visualization.about_app import show_about
...
class AppMenu():
        def __init__(self,master):
            ...
            self.button_5 = menu.add_cascade("About")
            ...
            dropdown5.add_option(option="PyLC",command= partial(show_about,master ))
        ...
        def connectExportJson(self, function):
            self.dropdown1.add_option(option="Save ladder", command=function )

        def connectImportJson(self, function):
            self.dropdown1.add_option(option="Load ladder", command=function )
